...
var m = [40, 0, 10, 0],
    z = width - m[1] - m[3],
    v = height - m[0] - m[2],
    xscale = d3.scale.ordinal().rangePoints([14, z], 1),
    yscale = {},
    dragging = {},
    line = d3.svg.line(),
...
var colors = {'quadruped': [31,119,180],
 'bipedal_tailed': [255,127,14],
 'insectoid':  [44,160,40],
 'serpentine_body': [214, 39, 40],
 'four_wings': [148,103,189],
 'two_wings': [140,86,75],
 'bipedal_tailless': [227,119,194],
 'head_legs': [188,189,34],
 'head_base': [41,75,61],
 'multiple_bodies':[60,86,61],
 'several_limbs': [30,100,73],
 'head_arms':  [20,49,49],
 'with_fins':  [359,69,49],
 'head_only':[185,80,45],
...
d3.csv("data/spk2.csv", function(raw_data) {
  ...
  g.append("svg:g")
      .attr("class", "brush")
      .each(function(d) { d3.select(this).call(yscale[d].brush = d3v3.svg.brush().y(yscale[d]).on("brush", brush)); })
    .selectAll("rect")
    .style('display',null)
...
function update_ticks(d, extent) {
  ...
  d3.selectAll(".axis")
    .each(function(d,i) {
      ...
      // bring lines back
      d3.select(this).selectAll('line').transition().delay(500).style("display", null);
...
function show_ticks() {
  d3.selectAll(".axis g").style("display", null).style('opacity',1);
  d3.selectAll(".axis path").style("display", null).style('opacity',1);
